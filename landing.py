...
def login_popup():
    ...
    # Side-by-side buttons for a choice, not a demand
    col1, col2 = st.columns(2)
    
    with col1:
        if st.button("Create Free Account", type="primary", use_container_width=True):
            # Route to your login/register page
...
def landing_page():
    ...
    with col_btn1:
        # Capture the button click in a variable
        explore_clicked = st.button("Explore Dashboard →", type="primary", use_container_width=True)
        
        if explore_clicked:
            # Check the session state you initialized in app.py
    ...
    with col_btn2:
        if st.button("Learn More", use_container_width=True):
            # Optional: Scroll down or route to a documentation page
            pass
